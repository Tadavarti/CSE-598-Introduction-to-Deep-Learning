...
def test_sigmoid(a):
    ...
    * it is  strictly increasing.
    ...
    if (a == 0):
        assert eq(sigmoid(0) , .5) == 1
    '''assert (1 if (0<=sigmoid(a) and sigmoid(a)<=1) else 0) ==1
    if (sigmoid(a)!=0 and sigmoid(a)!= 1):
        if (sigmoid(a)<.5):
            assert eq(1-sigmoid(a), sigmoid(a)) ==1'''
...
def test_other():
    ...
    for a in range(-100, 100):
        for b in range(-100, 100):
            for c in range(-100, 100):
                assert eq(mul(max(a , b) , c) , mul(c , max(b , a))) == 1
    None
...
def test_sum_distribute(ls1 , ls2):
    ...
    print(ls1)
    print(ls2)
    lsa = addLists(ls1 , ls2)
    print(lsa)
    lsb = lsa
    for i in range(len(lsb)):
        lsb[i] = 0
    for i in range(len(ls1)):
        lsb[i] = float(ls1[i]) + float(ls2[i])
    print(lsb)
    for i in range(len(lsa)):
        assert eq(lsa[i] , lsb[i]) == 1
