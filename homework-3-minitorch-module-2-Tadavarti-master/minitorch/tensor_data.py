...
def shape_broadcast(shape1, shape2):
    ...
    for i in range(abs(len(shape1) - len(shape2)) - 1, -1, -1):
        u_shape.append(large_shape[i])
    return tuple(u_shape[::-1])
    raise NotImplementedError('Need to implement for Task 2.2')
...
class TensorData:
    def __init__(self, storage, shape, strides=None):
        ...
        if strides is None:
            strides = tuple(strides_from_shape(shape))
    ...
    def permute(self, *order):
        ...
        return TensorData(self._storage, new_shape, strides=new_stride)
        raise NotImplementedError('Need to implement for Task 2.1')
